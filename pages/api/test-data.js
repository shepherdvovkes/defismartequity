...
const handler = withFullSecurity(
  withRateLimit({ maxRequests: 10, windowMs: 15 * 60 * 1000 })(
    withAuth(async (req, res) => {
      if (req.method === 'GET') {
        // GET - return information about available test data
      ...
      try {
        ...
        await db.log('error', 'Test data population error', { error: error.message });
        res.status(500).json({ error: 'Failed to populate test data', details: error.message });
      }
    })
  ),
  {
